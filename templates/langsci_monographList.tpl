...
<div class="cmp_monographs_list">
	...
	{if $featured && count($featured) > 0}
		{foreach from=$featured key=id item=array_key}
			{if isset($monographs[$id])}
				{include file="../plugins/generic/bookPage/templates/langsci_monograph_summary.tpl" monograph=$monographs[$id] isFeatured=1}
			{/if}
		{/foreach}
	{/if}
	...
	{foreach name="monographListLoop" from=$monographs item=monograph key=key}
		...
		{if $counter is odd by 1}
			<div class="row">
		{/if}
			{include file="../plugins/generic/bookPage/templates/langsci_monograph_summary.tpl" monograph=$monograph}
